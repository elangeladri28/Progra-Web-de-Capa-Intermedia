...
<script type="module">
    ...
    $(document).ready(function() {
        debugger
        var nombreCursoBuscaste = <?php echo json_encode($_POST["searchCourse"]); ?>;
        LosCursosBuscados(nombreCursoBuscaste);

        function LosCursosBuscados(nombreCursoBuscaste) {
            // Objeto en formato JSON el cual le enviaremos al webservice (PHP)
            var dataToSend = {
                nombre: nombreCursoBuscaste
            };
            var dataToSendJson = JSON.stringify(dataToSend);
            debugger
            var promise = $.ajax({
                url: urlglobal.url + "/LosCursosBuscados",
                async: true,
                type: 'POST',
                data: dataToSendJson,
                dataType: 'json',
